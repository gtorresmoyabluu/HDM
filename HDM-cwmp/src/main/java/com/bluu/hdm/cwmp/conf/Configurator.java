...
import com.bluu.hdm.cwmp.Application;
...
import com.bluu.hdm.cwmp.vendors.Vendor;
import com.bluu.hdm.cwmp.beans.ATMErrorsStatsLocal;
import com.bluu.hdm.cwmp.beans.ATMErrorsStatsLocalHome;
import com.bluu.hdm.cwmp.beans.BackupLocal;
import com.bluu.hdm.cwmp.beans.BackupLocalHome;
import com.bluu.hdm.cwmp.beans.ConfigurationLocal;
import com.bluu.hdm.cwmp.beans.ConfigurationLocalHome;
import com.bluu.hdm.cwmp.beans.DSLStatsLocal;
import com.bluu.hdm.cwmp.beans.DSLStatsLocalHome;
import com.bluu.hdm.cwmp.beans.DeviceProfile2SoftwareLocal;
...
import com.bluu.hdm.cwmp.beans.HostPropertyLocal;
import com.bluu.hdm.cwmp.beans.HostsLocal;
import com.bluu.hdm.cwmp.beans.ProfilePropertyLocal;
import com.bluu.hdm.cwmp.beans.ScriptLocal;
import com.bluu.hdm.cwmp.beans.ServiceLocal;
import com.bluu.hdm.cwmp.beans.ServicePropertyLocal;
import com.bluu.hdm.cwmp.beans.SoftwareDetailLocal;
import com.bluu.hdm.cwmp.beans.SoftwareDetailLocalHome;
import com.bluu.hdm.cwmp.beans.SoftwareLocal;
import com.bluu.hdm.cwmp.beans.SoftwareLocalHome;
import com.bluu.hdm.cwmp.datamodel.Parameter;
import com.bluu.hdm.cwmp.datamodel.Type;
import com.bluu.hdm.cwmp.interfaces.ICpe;
import com.bluu.hdm.cwmp.js.Script;
import com.bluu.hdm.cwmp.message.AddObject;
import com.bluu.hdm.cwmp.message.AddObjectResponse;
import com.bluu.hdm.cwmp.message.DeleteObject;
import com.bluu.hdm.cwmp.message.Download;
import com.bluu.hdm.cwmp.message.Fault;
import com.bluu.hdm.cwmp.message.GetParameterNames;
import com.bluu.hdm.cwmp.message.GetParameterNamesResponse;
import com.bluu.hdm.cwmp.message.GetParameterValues;
import com.bluu.hdm.cwmp.message.GetParameterValuesResponse;
import com.bluu.hdm.cwmp.message.GetRPCMethods;
import com.bluu.hdm.cwmp.message.GetRPCMethodsResponse;
import com.bluu.hdm.cwmp.message.Inform;
import com.bluu.hdm.cwmp.message.Message;
import com.bluu.hdm.cwmp.message.Reboot;
import com.bluu.hdm.cwmp.message.SetParameterValues;
import com.bluu.hdm.cwmp.message.SetParameterValuesResponse;
import com.bluu.hdm.cwmp.message.TransferComplete;
import com.bluu.hdm.cwmp.message.X_00000C_SetConfiguration;
import com.bluu.hdm.cwmp.tree.DataModelNode;
import com.bluu.hdm.cwmp.utils.Version;
    ...
    public void run() {
        try {
            ...
            try {
                ...
                log(Level.ERROR, "Device profile '" + host.getProfileName() + "' not found.", ex);
            }
...
//                GZIPOutputStream gz = new GZIPOutputStream(os);
            ...
            log(Level.WARN, "Failed get software parameter names: " + m.toString());
        }
...
//                GZIPOutputStream gz = new GZIPOutputStream(os);
            ...
            log(Level.WARN, "Failed get software RPC methods: " + m.toString());
        }
            ...
            if (m instanceof GetParameterNamesResponse) {
                ...
                for (String name : gpnr.names.keySet()) {
                    ...
                    if (m instanceof GetParameterValuesResponse) {
                        ...
                        log(Level.WARN, "Failed save CWMP vars for +'" + name + "': " + m.toString());
                        Fault f = (Fault) m;
                ...
                log(Level.WARN, "Failed save CWMP vars: " + m.toString());
            }
        } catch (Exception ex) {
            log(Level.WARN, "Failed save CWMP vars: " + ex.getMessage());
        }
    }
...
//                    DownloadConfig(cfg, null);
            ...
            log(Level.WARN, "Config lookup: " + e.getMessage());
        } catch (Exception e) {
            log(Level.ERROR, e.getMessage());
        }
    }
...
//                if (!version.equals(lastInform.getProvisiongCode())) {
    ...
    private boolean checkImage(String name) {
        ...
        if (!f.exists()) {
            log(Level.WARN, "File not present for firmware version " + filename);
            return false;
        }
        if (!f.canRead()) {
            log(Level.WARN, "File not readable for firmware version " + filename);
            return false;
        }
        return true;
    }
...
//            System.out.println ("Profile sets firmware version to "+dp2sl.getVersion());
            ...
            if (!versionProfile.equals(DeviceProfile2SoftwareLocal.AUTOUPDATE)) {
                ...
                /*
                 ...
                 swUpdate = null;
                 log(Level.WARN, "File not present for firmware version " + swUpdate.getVersion());
                 return;
                 }
                 */
...
//                    d.TargetFileName = swUpdate.getFileName ();
            ...
            log(Level.WARN, "Software lookup: " + e.getMessage());
//                ex.printStackTrace();
        }
    }
...
//        return version;
    ...
    private void runScript(String scriptName, TransferComplete tc) {
        ...
        try {
            ...
            log(Level.WARN, "Configuration script '" + scriptName + "' not found in db.");
        }
    }
...
//                        System.out.println ("n="+n+" v="+is);
    ...
    private void _CheckParameters() {
        ...
        if (m instanceof GetParameterValuesResponse) {
            ...
            log(Level.ERROR, (m != null) ? m.toString() : "Getting Periodic inform timeout");
        }
    }
    ...
    private void UpdateVoiceCaps() {
        ...
        if (m instanceof GetParameterValuesResponse) {
            ...
            for (Map.Entry<String, String> e : gpvr.values.entrySet()) {
            ...
            //logger.log(Level.WARN, "Failed get voice service caps: " + m.toString());
    ...
    protected boolean SyncCwmpVariables(boolean fForcePasswords) {
        ...
        try {
            if (!getProfileProps(p, root, host.getProfileName())) {
                log(Level.ERROR, "Profile loop");
                return false;
            }
        } catch (FinderException ex) {
            log(Level.ERROR, "Profile not found", ex);
            return false;
        }
...
//                p.setProperty(name, value);
        ...
        for (String par : params) {
            if (p.getProperty(getNormalizedName(root, par)) == null) {
                log(Level.WARN, "Parameter " + par + " should be overriden in host properties.");
                fMissingParams = true;
            }
        }

        if (fMissingParams) {
            log(Level.WARN, "Not all required parameters set.");
            return false;
        }
        ...
        try {
            for (DataModelNode c : l) {
                ...
                if (c.isMultiInstance()) {
                    ...
                    if (d == null) {
                        log(Level.WARN, "Var " + fqdnCanonic + " is not known in dictionary");
                        continue;
                    }
                    ...
                    if (d.isWritable() && !c.DoNotCreateDelete()) {
                        ...
                        if (cpenames.length != chknames.length) {
                            log(Level.WARN, "Var " + fqdn + " is not writable and var count (" + chknames.length + ")!= count on cpe(" + cpenames.length + ")");
                        }
                    }
