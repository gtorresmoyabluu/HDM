...
import com.bluu.hdm.rest.vo.administration.UserVO;
...
public class UserService {
	...
	entity.setLastModification(java.sql.Date.valueOf(LocalDate.now()));
        entity.setBackupPswd(CryptoUtils.encrypt(userRequest.getBackupPswd()));
        entity.setPassword(CryptoUtils.encodePassword(userRequest.getBackupPswd()));
    ...
    public ResponseEntity<UserVO> updateUser(@RequestBody UserVO userRequest) {
	System.out.println("Updating User " + userRequest.getId());
	if (!userService.isExist(userRequest.getId())) {
	    logger.error("User with id " + userRequest.getId() + " not found");
	...
	try {
		...
		source.forEach((UserEntity userEntity) -> {
                    UserVO user = mapper.convertValue(userEntity, UserVO.class);
		    response.add(user);
		});
