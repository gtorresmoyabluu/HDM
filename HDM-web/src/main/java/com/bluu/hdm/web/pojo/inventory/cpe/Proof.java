...
public class Proof extends Evaluable implements Serializable {
    ...
    public Proof(Proof prf) {
        super(prf);
        for (Action act : prf.actLst) {
            actLst.add(new Action(act));
        }
        for (Message1 msg : prf.msgLst) {
    ...
    public String toString() {
        String result = "Prf Id: " + id;
        if (boolEvl != null) {
            result += "\nPrf Boolean AndType: " + boolEvl.isAndType() + ", Result: " + boolEvl.getResult() + "\n" + boolEvl;
